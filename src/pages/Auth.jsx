  ...
  return (
    <div className={styles.page}>
      <div className={styles.tabRow}>
        ...
        <button className={`${styles.tab} ${tab === 'signup' ? styles.activeTab : ''}`} onClick={() => setTab('signup')}>Sign Up</button>
      </div>
        ...
        <Card>
          ...
          <form onSubmit={handleLogin} className={styles.form}>
            ...
            <div className={styles.loginRow}>
              <button type="button" className={styles.forgotLink} onClick={handleReset}>Forgot password?</button>
        ...
        <Card>
          ...
          <form onSubmit={handleSignup} className={styles.form}>
            <Input label="Full Name" placeholder="Your Name"
              value={signupForm.name} onChange={e => setSignupForm(f => ({ ...f, name: e.target.value }))} required />
            <Input label="Email" type="email" placeholder="you@example.com"
              value={signupForm.email} onChange={e => setSignupForm(f => ({ ...f, email: e.target.value }))} required />
            <Input label="Password" type="password" placeholder="At least 8 characters"
              value={signupForm.password} onChange={e => setSignupForm(f => ({ ...f, password: e.target.value }))} required />
            <label className={styles.checkboxLabel}>
              <input type="checkbox" checked={signupForm.agree}
            ...
            {signupMsg && <div className={styles.successMsg}>{signupMsg}</div>}
